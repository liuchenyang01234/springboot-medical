...
$(function(){
	...
	info_format	: function(value) {
		...
		switch (value) {
			case 1 : 
				info	= '<div class="info-box">1分&nbsp;很不满意<div>医生态度非常恶劣，太令人失望了！</div></div>';
				break;
			case 2 : 
				info	= '<div class="info-box">2分&nbsp;不满意<div>医生态度不太好，不符合我的期望。</div></div>';
				break;
			case 3 : 
				info	= '<div class="info-box">3分&nbsp;一般<div>医生态度感觉一般。</div></div>';
				break;
			case 4 : 
				info	= '<div class="info-box">4分&nbsp;满意<div>医生态度较好，符合我的期望。</div></div>';
				break;
			case 5 : 
				info	= '<div class="info-box">5分&nbsp;非常满意<div>医生态度非常好，耐心回答我的所有问题。</div></div>';
				break;
			default :
				info = value;
				break;
...
function result() {
    var report =  $("input:checkbox[name='report']:checked").val();
    var score = $(".rater-star-result").text().substring(0,1);
	// 1:投诉
    if (report === "on"){
        report = 1;
        score = 0;
        //$("#s_zero").css("display","block");
	} else{
        report = 0;
	}
	$.ajax({
        type: "POST",
        url: config.base_url + "/Evaluation/create",
        data: {
            'token': checktoken(),
